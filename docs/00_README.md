...
> **本リポジトリでの適用状況（2026-09-10 完了）**: 架空案件（コミュナレ）のサンプルはすべて削除済み。**中身を伴う SAMPLE ブロックは `docs/` 配下に 1 つも残っていない**（本節 §5-1 の記法説明を除く）。埋めた箇所は naturaling.jp のリポジトリとサイトのコンテンツから確認できる事実のみで、根拠のない箇所は **要記入** / **該当なし** と明示してある。本節はテンプレートを別案件へ再利用する際の参照として残す。
...
> 上のコードブロックは記法の説明であり、SAMPLE ブロックそのものではない。
