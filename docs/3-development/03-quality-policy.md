...
### 3-1b. 診断プラットフォームで必ず書くテスト（設計済み — DEV-11 §10）

| 対象 | 種別 | 必ず検証すること | Phase |
| --- | --- | --- | :---: |
| 無料診断 判定 v2（`business` / `ai-dx` の `scoring.ts`） | 単体（**Node、workerd 不要**。`apps/public/vitest.config.ts` に D1 を使わない第 2 プロジェクトを足す） | 全回答組み合わせのシミュレーション（タイプ別出現率・Z 出現率・同点率を出力）、該当なし除外、強み・判定理由、v1 と v2 の結果 URL 生成 | 2a |
| 結果ページ | E2E | 15 本の結果 URL が props 付きで prerender される、`v` 無し URL が v1 描画、パラメータ欠落時の非表示、タイプ別 CTA の遷移先、GA4 `dataLayer` への push | 2a |
| トークン | 単体（workerd） | 期限・回数上限・失効で 404、`first_clicked_at` の初回のみ記録、回答保存で `use_count` と `expired` が同一 batch | 2b |
| 回答保存 | 単体 | サーバー再計算とクライアント結果の不一致がフラグに残る、定義の版が違えば 422、`lead_id` 無しの行が企業名と結合されない（Service の戻り値に会社名が無い） | 2b |
| リード | 単体 | Turnstile 失敗で作られない、`purpose = briefing` で `briefing_requests` が同時に作られる、遷移マトリクス全網羅 | 2b |
| 有料診断 | 単体 | 遷移マトリクス全網羅、承認前の納品が 409、`answering` 以外での回答保存が 409、提出時の必須未回答が 422、本人以外の GET が 404 | 3 |
| 有料版ロジック（PRD-09） | 単体（Node） | 分野スコア・レベル境界値、緊急度、優先候補 8 件の並び、矛盾ルール R-01〜R-12 の全件 | 3 |
| AI 後処理 | 単体 | モック出力で根拠 ID 検証・候補外の警告化・スキーマ不一致の再生成 1 回・`refusal` の failed、月次上限で 422 | 3 |
| パートナー分離 | 単体 | すべての partner Service で別パートナー ID が 404、`suspended` で全セッション削除、同意無しの回答保存が 409 | 4 |
| 案件・手数料 | 単体 | チェックリスト未達の提出が 422、`confirm-commission` の額計算、`editor` の確定が 403、リクエストの `commission_amount` が無視される、確定取消の 409 条件 | 4 |
| 管理画面 | E2E | ログイン → 承認 → 納品、案件受付 → 確定の主要フロー。新規 admin ルートを 401 リストに追加（`scaffold` §4） | 3 / 4 |

判定関数の単体テストは D1 を使わないため、`tests/setup.ts` のマイグレーション必須チェックに引っかからない構成（vitest の `projects`）にする。**設計フェーズの現在は 1 本も存在しない**（PRD-06 P-19）。
