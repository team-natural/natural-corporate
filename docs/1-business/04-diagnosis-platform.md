...
| --- | --- | --- |
| 内部呼称 | 完全版 | Confirmed |
| 対外名称 | **中小企業向け IT・DX現状診断**（GOV-01 D-014。候補だった IT・DX総合診断 / 経営者のためのIT・DX診断は不採用） | Decided |
| 価格 | **モニター 5〜10 社は 30,000 円（税別）→ 本価格 50,000 円（税別）**。運用時間の実測後に移行（GOV-01 D-015）。価格は環境変数で保持 | Decided |
...
> **2026-09-24 追記（GOV-01 D-012）**: Phase 1 の詳細設計として、PRD-01〜04・PRD-09（有料版質問）・DEV-02/03/04/07/08/09/10・OPS-01/02 へ診断プラットフォームの定義を追加済み。実装の作業分解は DEV-11 §12。正仕様承認（GOV-01 §4）後にフェーズ順で実装する。
