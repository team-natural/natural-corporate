...
### D-016：business v2 の配点と I タイプ名を PRD-07 案のまま確定

| 項目 | 内容 |
| --- | --- |
| 日付 | 2026-09-26 |
| カテゴリ | プロダクト |
| 決定内容 | 業務課題かんたん診断 v2 の配点は PRD-07 §1-2 の値で確定（最大点 A5 / B3 / C5 / D3 / E5 / F3 / G3 / H5 / I2）。I タイプの名称は「顧客リピート不足型」（URL `i` は不変） |
| 背景 | 全組み合わせシミュレーション（一様分布で I 34%・B 29%・同点 41% — `docs/WIP-diagnosis-platform-tasks.md` §6）を見たうえで、実回答の分布を運用で確認する方針。同点は q0 の関心と固定順で解決される |
| 影響範囲 | `apps/public/src/diagnoses/business/data.ts`（実装済み）、`tests/scoring/business.test.ts` の最大点期待値、PRD-07 §1-2・§1-4 |
| 決定者 | 藤本 翔太 |
| 関連 TBD | TBD-21, TBD-22 |

### D-017：15 分解説の予約ツールは Google カレンダーの予約スケジュール

| 項目 | 内容 |
| --- | --- |
| 日付 | 2026-09-26 |
| カテゴリ | プロダクト・運用 |
| 決定内容 | D-011 の外部予約ツールとして Google カレンダーの予約スケジュール（Google Workspace）を使う。予約ページ URL を `PUBLIC_BRIEFING_BOOKING_URL`（public のビルド変数）に設定する。備考欄のクエリ自動入力に対応しないため、結果 URL と回答 ID は受付確認メール（`mail/leads.ts`）と管理画面（ADM-12）で参照する |
| 背景 | 既存の Google Workspace で完結し、追加費用と契約が要らない |
| 影響範囲 | DEV-01 §2、DEV-10 §8-6、DEV-08 §8-1、`apps/public/.dev.vars.example` |
| 決定者 | 藤本 翔太 |
| 関連 TBD | TBD-25 |

### D-018：有料診断の決済は請求書払いと Stripe の両方

| 項目 | 内容 |
| --- | --- |
| 日付 | 2026-09-26 |
| カテゴリ | 事業・設計 |
| 決定内容 | 申込時に請求書払い / カード決済（Stripe）を選べる。請求書は手動の入金確認（admin の `confirm-payment`）、Stripe は Webhook で `paid` へ自動遷移。`paid_diagnoses.payment_method` に記録する |
| 背景 | モニター段階から顧客の選択肢を残す。DEV-10 §2 の Stripe 標準を採用 |
| 影響範囲 | DEV-07 §5-9（`payment_method`、`stripe_event_logs`）、DEV-09 §2-8、DEV-10 §2、DEV-04 §5-7、PRD-04 SCR-22 |
| 決定者 | 藤本 翔太 |
| 関連 TBD | TBD-27 |

### D-019：AI 分析は Claude（Opus 5 / Sonnet 5）

| 項目 | 内容 |
| --- | --- |
| 日付 | 2026-09-26 |
| カテゴリ | 技術スタック・AI |
| 決定内容 | 有料診断の AI 分析は Vercel AI SDK + `@ai-sdk/anthropic`。分析本体は `claude-opus-5`、要約・整形など軽い工程は `claude-sonnet-5`。モデル ID は `prompts.model_id` に持ち、環境変数で既定値を上書きできるようにする。契約時に単価・学習利用・保持期間・リージョンを公式規約で確認する |
| 背景 | DEV-01 §2 の統合レイヤー標準と一致。根拠 ID 付きの構造化出力に適する |
| 影響範囲 | DEV-01 §2、DEV-10 §5、PRD-05、`apps/admin` のパッケージ追加と Secrets（`ANTHROPIC_API_KEY`） |
| 決定者 | 藤本 翔太 |
| 関連 TBD | TBD-28 |

### D-020：診断担当者は `editor` ロールを流用

| 項目 | 内容 |
| --- | --- |
| 日付 | 2026-09-26 |
| カテゴリ | 認証・権限 |
| 決定内容 | 有料診断の編集（回答確認・AI 分析の編集・差戻し依頼）は `editor`、承認・入金確認・納品・取消は `admin`。新ロールは作らない |
| 背景 | テンプレート標準の 2 ロールを維持し、00_README §0-1 の適合性再確認を不要にする |
| 影響範囲 | DEV-02 §2 マトリクス、DEV-04 §5-9 |
| 決定者 | 藤本 翔太 |
| 関連 TBD | TBD-29 |

### D-021：有料版 PDF は Cloudflare Browser Rendering で生成し R2 に保存

| 項目 | 内容 |
| --- | --- |
| 日付 | 2026-09-26 |
| カテゴリ | 技術スタック |
| 決定内容 | 印刷用テンプレート（`/print/report/<id>/`）を Browser Rendering で PDF 化し、R2 に保存して `reports` 行で管理する。ダウンロードは admin 側の署名付き配信。Phase 4 のパートナー版簡易 PDF も同じ基盤 |
| 背景 | 納品物として体裁を揃える。DEV-01 §2 の標準 |
| 影響範囲 | admin `wrangler.jsonc`（`browser` バインディング・R2 実値化）、DEV-07 §5-11、DEV-10 §8-5 |
| 決定者 | 藤本 翔太 |
| 関連 TBD | TBD-30 |

### D-022：AI 送信に同意しない顧客は AI 分析を省いて同価格・納期延長

| 項目 | 内容 |
| --- | --- |
| 日付 | 2026-09-26 |
| カテゴリ | 事業 |
| 決定内容 | 申込時の AI 送信同意は任意。未同意の案件はロジック判定 + 専門家の手書き分析で納品し、価格は同じ、納期が延びる旨を申込時に明示する。`paid_diagnoses.consent_ai_at` が NULL の案件では `ai-jobs` を 409 で拒否する |
| 背景 | 顧客の選択権を残し、AI 不能時の運用と同じ経路で処理できる |
| 影響範囲 | PRD-04 SCR-22、DEV-04 §5-9、DEV-09 §2-8、OPS-01（規約） |
| 決定者 | 藤本 翔太 |
| 関連 TBD | TBD-32 |
