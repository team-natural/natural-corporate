...
const { slug, questions, axes, thresholds } = questionsData;
...
function finish() {
  const result = computeResult({ questions, axes, thresholds }, answers);

  const params = new URLSearchParams();
  params.set("s", result.axisScores.join("."));
  window.location.href = `${diagnosisResultPath(slug, `level-${result.level}`)}?${params.toString()}`;
}
